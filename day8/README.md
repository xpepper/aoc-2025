...
To save on string lights, the Elves would like to focus on connecting pairs of junction boxes that are _as close together as possible_ according to straight-line distance. In this example, the two junction boxes which are closest together are `162,817,812` and `425,690,689`.
...
1. Parsing 3D coordinates from the input
2. Calculating distances between all pairs of junction boxes
3. Using a Union-Find (Disjoint Set Union) data structure to track circuits
4. Connecting pairs in order of increasing distance (skipping pairs already in the same circuit)
5. After 1000 connections, finding the sizes of all circuits
6. Multiplying the three largest circuit sizes
