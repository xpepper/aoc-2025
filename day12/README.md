...
First, it contains a list of the presents' shapes. Second, it contains the size of the region under each tree and a list of the number of presents of each shape that need to fit into that region. For example:
...
In the example, there are six unique present shapes and three regions that need checking.

1. **First region (4x4)**: Need to fit two presents with shape index `4`. This is possible.

2. **Second region (12x5)**: Need to fit one present with shape index `0`, one with shape index `2`, two with shape index `4`, and two with shape index `5`. This is possible.

3. **Third region (12x5)**: Same as second but with one additional present with shape index `4`. This is NOT possible.
...
Part 2 description will be available after solving Part 1.
